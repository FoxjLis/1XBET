...
async def send_last_news(sport)->list:
    if sport == 'football':
...
async def do_parser_news(*args) -> None:
    ...
    async def edit_message() -> None:
        ...
        if callback.data == 'footbal' and len(news_set_foot) > 6 and users_news[user_id][0]:
            await callback.bot.send_message(user_id, 'Актуальные новости по футболу:\n' + news_text_f,
                                            reply_markup=kb.go_menu)
            news_set_foot.clear()

        if callback.data == "volleyball" and len(news_set_volley) > 6 and users_news[user_id][2]:
            await callback.bot.send_message(user_id, 'Актуальные новости по волейболу:\n' + news_text_v,
                                            reply_markup=kb.go_menu)
            news_set_volley.clear()
        if callback.data == "hockey" and len(news_set_hockey) > 6 and users_news[user_id][1]:
            await callback.bot.send_message(user_id, 'Актуальные новости по хоккею:\n' + news_text_h,
                                            reply_markup=kb.go_menu)
